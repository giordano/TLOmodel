...
def test_integrity_of_linear_models(tmpdir):
    ...
    for pathogen in alri.all_pathogens:
        ...
        # assert (res > 0).all() and (res <= 1.0).all()   # Todo: error on res<=1.0
...
def test_nat_hist_recovery(tmpdir):
    ...
    sim.modules['Alri'].models.death_risk = death
...
def test_treatment(tmpdir):
    ...
    sim.modules['Alri'].do_treatment(person_id=person_id, appointment_type='referred', facility_level='2')
...
def test_use_of_HSI(tmpdir):
    ...
    # Run the HSI event
    hsi = HSI_iCCM_Pneumonia_Treatment(person_id=person_id, treatment_plan='treat_non_severe_pneumonia',
                                       appointment_type='first_appointment', module=sim.modules['Alri'])
