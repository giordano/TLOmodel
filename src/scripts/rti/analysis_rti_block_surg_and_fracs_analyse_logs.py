...
mean_dalys_per_scenario = [np.mean(normal_run_dalys), np.mean(no_cast_dalys), np.mean(no_minor_dalys),
                           np.mean(no_major_dalys), np.mean(no_hs_dalys)]
scenarios = ['Normal', 'No fracture\ncasts', 'No minor\nsurgery', 'No major\nsurgery', 'No health\nsystem']
plt.bar(np.arange(len(mean_dalys_per_scenario)), mean_dalys_per_scenario,
        color=['lightsteelblue', 'lightsalmon', 'thistle', 'peachpuff', 'mediumpurple'])
...
colors = ['lightsteelblue', 'lightsalmon', 'thistle', 'peachpuff', 'mediumpurple']
...
plt.bar([2], mean_dalys_per_scenario[-1], color=colors[-1], label='no health\nsystem')
plt.ylabel('DALYs')
plt.legend()
plt.savefig('C:/Users/Robbie Manning Smith/Pictures/TLO model outputs/BlockedInterventions/'
