...
```tlo batch-submit src/scripts/healthsystem/impact_of_cons_availability/impact_of_pharmacist_intervention_hard_hr_constraints.py```

or locally using:
    ```tlo scenario-run src/scripts/healthsystem/impact_of_cons_availability/impact_of_pharmacist_intervention_hard_hr_constraints.py```
...
class ImpactOfConsumablesAvailabilityIntervention(BaseScenario):
    def __init__(self):
        ...
        self.end_date = Date(2011, 2, 1) # 31/12/2025
        self.pop_size = 100 # large population size for final simulation = 100,000
        self.number_of_draws = 2  # <- one scenario
        self.runs_per_draw = 3  # <- repeated this many times = 5
    ...
    def draw_parameters(self, draw_number, rng):
        return {
            'HealthSystem': {
                ...
                'mode_appt_constraints': [2,2][draw_number]
               }
        }
