...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "Analyse scenario results for baseline pathways"
    )
    parser.add_argument(
    ...
    results_folder = get_scenario_outputs("impact_of_baseline_Tb_diagnosis_pathways.py", outputspath)[-1]
   ...
   # print(f" the log is {log['tlo.methods.population']['scaling_factor'].keys()}")
    ...
    total_deaths.to_excel(outputspath / "deaths_baseline.xlsx")
    total_dalys = extract_total_dalys(results_folder)
    print(f"these are expected dalys draw {total_dalys}")
    total_dalys.to_excel(outputspath / "dalys_baseline.xlsx")
...
#print(f"The log keys for 'demography' are: {list(log['tlo.methods.demography'].keys())}")
