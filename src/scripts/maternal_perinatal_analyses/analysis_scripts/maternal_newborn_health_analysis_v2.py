...
def run_maternal_newborn_health_analysis(scenario_file_dict, outputspath, intervention_years, service_of_interest,
    ...
    births_dict = analysis_utility_functions.return_birth_data_from_multiple_scenarios(results_folders,
                                                                                       intervention_years)
    # todo: births in each scenario are EXACTLY the same?
    ...
    for data, colour, title, y_lable in \
        zip(['agg_dir_m_deaths', 'agg_dir_mr', 'agg_ind_m_deaths', 'agg_ind_mr', 'agg_n_deaths', 'agg_nmr'],
            ['yellowgreen', 'powderblue', 'yellowgreen', 'powderblue', 'yellowgreen', 'powderblue'],
        ...
        plt.bar(labels, mean_vals, color=colour, width=0.4, yerr=ci)
        plt.xlabel("Scenario")
        plt.ylabel(y_lable)
        plt.title(title)
    ...
    # def get_deaths_by_pregnancy_period(results_folder, intervention_years):
    #
    #     i_d = extract_results(
    #         results_folder,
    ...
    #             lambda df: df.loc[df['is_pregnant']].assign(
    #                 year=df['date'].dt.year).groupby(['year'])['year'].count()))
    #     indirect_deaths = i_d.fillna(0)
    #
    # death_timing_data = {k: get_deaths_by_pregnancy_period(results_folders[k], intervention_years) for k in
    #                      results_folders}
    ...
    def extract_deaths_by_cause(results_folder, intervention_years, scenario_name, final_df):
        ...
        # for comp in direct_deaths.index:
        #     data = [direct_deaths.loc[comp].mean(),
        #             direct_deaths.loc[comp].quantile(0.025),
        #             direct_deaths.loc[comp].quantile(0.925)]
        #     mean_ds_by_cause.update({comp: data})
        #
        # for comp in indirect_deaths.index:
        #     data = [indirect_deaths.loc[comp].mean(),
        #             indirect_deaths.loc[comp].quantile(0.025),
        #             indirect_deaths.loc[comp].quantile(0.925)]
        #     mean_ds_by_cause.update({comp: data})
    ...
    if (service_of_interest != 'pnc') or show_all_results:
        ...
        for data, colour, title, y_lable in \
            zip(['agg_sb', 'agg_an_sb', 'agg_ip_sb'],
                ['yellowgreen', 'powderblue', 'steelblue'],
