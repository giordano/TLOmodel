...
path = f'{output_path}/sensitivity_analysis_{results_folder.name}'
...
def output_comparison_graph_for_given_param_change(param, target_m, cause_dict):
    ...
    s_pval = [f'{val}({x})' for val, x in zip(p_vals, ['a', 'b', 'c', 'd', 'e', 'f'])]
...
p_dict = {'Labour:prob_hcw_avail_man_r_placenta': 'pph',
          'Labour:prob_hcw_avail_blood_tran': 'pph',
          'Labour:prob_hcw_avail_anticonvulsant': 'eclampsia',
          'Labour:treatment_effect_modifier_one_delay':  'all',
          'Labour:treatment_effect_modifier_all_delays': 'all',
          'Labour:mean_hcw_competence_hp': 'all'}

for k in p_dict:
    output_comparison_graph_for_given_param_change(k, p_dict[k], d_by_c_dfs_dict)
