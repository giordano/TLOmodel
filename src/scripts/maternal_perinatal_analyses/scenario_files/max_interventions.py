...
class MaximumIntervention(BaseScenario):
    def __init__(self):
        super().__init__()
        self.seed = 55447
    ...
    def log_configuration(self):
        return {
            'filename': 'max_ints_200k', 'directory': './outputs',
            "custom_levels": {
                "*": logging.WARNING,
    ...
    def draw_parameters(self, draw_number, rng):
        return {'PregnancySupervisor': {'alternative_anc_coverage': True,
                                        'alternative_anc_quality': True,
                                        'anc_availability_odds': 25.0,
                                        'anc_availability_probability': 1.0,
                                        'full_service_availability': True,
                                        'analysis_year': 2021},

                'Labour': {'analysis_year': 2021,
                           'alternative_pnc_coverage': True,
                           'alternative_pnc_quality': True,
                           'pnc_availability_odds': 25.0,
                           'pnc_availability_probability': 1.0,
                           'alternative_bemonc_availability': True,
