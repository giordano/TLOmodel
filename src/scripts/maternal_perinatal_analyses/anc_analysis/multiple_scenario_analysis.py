...
def run_multiple_scenario_analysis(scenario_file_dict, outputspath, show_and_store_graphs, intervention_years,
    ...
    def get_crude_death_graphs_graphs(death_data):
        """
        ...
        def output(group):
            ...
            N = len(death_data['Status Quo'][f'crude_{l}_deaths'][0])  # todo: will crash if change the baseline name
            ind = np.arange(N)
            width = 0.2

            for k, position, colour in zip(death_data, [ind-width, ind, ind+width, ind+width*2],
                                           ['bisque', 'powderblue', 'mistyrose', 'thistle']):
                ci = [(x - y) / 2 for x, y in zip(death_data[k][f'crude_{l}_deaths'][2],
                                                  death_data[k][f'crude_{l}_deaths'][1])]
                plt.bar(position, death_data[k][f'crude_{l}_deaths'][0], width, label=k, yerr=ci, color=colour)
        ...
        for group in ['Maternal', 'Neonatal']:
            output(group)
    ...
    if show_and_store_graphs:
        ...
        get_crude_death_graphs_graphs(death_data)
    ...
    if show_and_store_graphs:
        ...
        N = len(sbr_data['Status Quo']['crude_sb'][0])  # todo: will crash if change the baseline name
        ind = np.arange(N)
        width = 0.2

        for k, position, colour in zip(sbr_data, [ind - width, ind, ind + width, ind + width * 2],
                                       ['bisque', 'powderblue', 'mistyrose', 'thistle']):
            ci = [(x - y) / 2 for x, y in zip(sbr_data[k]['crude_sb'][2], sbr_data[k]['crude_sb'][1])]
            plt.bar(position, sbr_data[k]['crude_sb'][0], width, label=k, yerr=ci, color=colour)

        plt.ylabel('Total Stillbirths (scaled)')
        plt.xlabel('Years')
        plt.title(f'Yearly Baseline Stillbirths Compared to Intervention')
        plt.legend(loc='best')
        plt.xticks([0., 1., 2., 3., 4., 5.], labels=intervention_years)
    ...
    # Get the data
    hcw_data = {k: get_hcw_time_per_year(results_folders[k]) for k in results_folders}
